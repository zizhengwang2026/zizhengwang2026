...
<td width="33%" align="center" valign="middle">
  <div style="background: linear-gradient(135deg, #EEF2FF, #C7D2FE); padding: 20px; border-radius: 18px; border: 2px solid #6366F1; box-shadow: 0 0 25px rgba(99,102,241,0.4);">
    <div style="font-size: 36px;">🤖</div>
    <div style="color: #26215C; font-weight: bold; font-size: 18px; margin-top: 4px;">Embodied AI</div>
    <div style="color: #534AB7; font-size: 11px; margin-top: 2px;">learning on real bodies</div>
  </div>
</td>
<td width="34%" align="center" valign="middle" rowspan="2">
  <div style="background: radial-gradient(circle, #6366F1 0%, #8B5CF6 50%, #1e1b4b 100%); padding: 30px; border-radius: 50%; width: 220px; height: 220px; box-shadow: 0 0 60px rgba(99,102,241,0.6); margin: 0 auto; text-align: center; box-sizing: border-box;">
    <img src="https://avatars.githubusercontent.com/zizhengwang2026" alt="me" width="90" style="border-radius:50%; border: 4px solid #ffffff; box-shadow: 0 0 20px rgba(255,255,255,0.3);"/><br/><br/>
    <span style="color:#ffffff; font-weight:bold; font-size: 16px;">Zizheng Wang</span><br/>
    <span style="color:#c7d2fe; font-size: 11px;">MSc · ZJU</span>
  </div>
</td>
<td width="33%" align="center" valign="middle">
  <div style="background: linear-gradient(135deg, #ECFEFF, #A5F3FC); padding: 20px; border-radius: 18px; border: 2px solid #06B6D4; box-shadow: 0 0 25px rgba(6,182,212,0.4);">
    <div style="font-size: 36px;">🧠</div>
    <div style="color: #04342C; font-weight: bold; font-size: 18px; margin-top: 4px;">AI Agents</div>
    <div style="color: #0F6E56; font-size: 11px; margin-top: 2px;">plan · reason · act</div>
  </div>
</td>
</tr>
<tr>
<td width="33%" align="center" valign="middle">
  <div style="background: linear-gradient(135deg, #F0FDF4, #BBF7D0); padding: 20px; border-radius: 18px; border: 2px solid #22C55E; box-shadow: 0 0 25px rgba(34,197,94,0.4);">
    <div style="font-size: 36px;">🦾</div>
    <div style="color: #173404; font-weight: bold; font-size: 18px; margin-top: 4px;">Soft Robotics</div>
    <div style="color: #3B6D11; font-size: 11px; margin-top: 2px;">compliant, morphologically smart</div>
  </div>
</td>
<td width="33%" align="center" valign="middle">
  <div style="background: linear-gradient(135deg, #FEFCE8, #FDE68A); padding: 20px; border-radius: 18px; border: 2px solid #F59E0B; box-shadow: 0 0 25px rgba(245,158,11,0.4);">
    <div style="font-size: 36px;">🦿</div>
    <div style="color: #412402; font-weight: bold; font-size: 18px; margin-top: 4px;">Humanoid Robots</div>
    <div style="color: #854F0B; font-size: 11px; margin-top: 2px;">full-body embodiment</div>
  </div>
</td>
</tr>
<tr>
<td colspan="3" align="center" valign="middle">
  <div style="background: linear-gradient(135deg, #FFF1F2, #FECDD3); padding: 20px; border-radius: 18px; border: 2px solid #F43F5E; box-shadow: 0 0 25px rgba(244,63,94,0.4); display: inline-block; min-width: 320px;">
    <div style="font-size: 36px;">⌚</div>
    <div style="color: #4B1528; font-weight: bold; font-size: 18px; margin-top: 4px;">Wearable Devices</div>
    <div style="color: #993556; font-size: 11px; margin-top: 2px;">soft exo-suits · rehab · skin-like sensing</div>
  </div>
</td>
</tr>
</table>
...
*My first end-to-end embodied AI pipeline. Not a simulation. Not a tutorial. Real hardware, real data, real policy.*

<div style="background: linear-gradient(135deg, #fef3c7, #fde68a); padding: 18px; border-radius: 12px; border-left: 5px solid #f59e0b; margin: 12px 0;">

**🎯 Mission:** Build a robotic arm that sees an object and grasps it autonomously — using imitation learning, no hand-coded control.

**⭐ Why it matters:** This is the canonical **"perceive → decide → act"** embodied-AI loop, run on physical hardware.
...
|---|---|---|
| **D-Robotics S600** | Edge AI compute | RDK board, aarch64 |
| **SO-101 Arms** × 2 | Leader + follower | 6-DOF, FEETECH servos |
| **Cameras** × 2 | Visual input | Dual-view workspace |
| **Host PC** | Teleoperation + training | Linux + Python |

#### 💻 Software Stack

| Layer | Tool |
|---|---|
| OS (board) | Debian (aarch64) |
| Env | Miniconda + HuggingFace **LeRobot** |
| Algorithm | **ACT** — Action Chunking Transformer |
| Compute | PyTorch |
| Calibration | FEETECH servo SDK |

#### 🔁 The 8-Step Pipeline (Run Hands-On)

```bash
# Step 1: Flash the S600 board
xburn → SSH in

# Step 2: Build a clean aarch64 environment
miniconda → pip install lerobot

# Step 3: Calibrate leader and follower arms
python -m lerobot.calibrate --id FEETECH --center --limits

# Step 4: Collect demonstration data
python -m lerobot.teleop --dual-camera --record

# Step 5: Audit collected episodes
python -m lerobot.replay --episode 1

# Step 6: Train ACT
python -m lerobot.train --algo ACT --epochs 50

# Step 7: Inspect loss curve
python -m lerobot.check --loss-curve

# Step 8: Deploy
python -m lerobot.policy_server &
python -m lerobot.act_client
```

#### 🐛 Bugs I hunted down (one by one)

| Problem | Why it happens | How I fixed it |
|---|---|---|
| SSH drops during flash | power glitch on USB-C | powered hub + retry loop |
| Conda installs x86 wheels on aarch64 | default channel wrong | pinned `aarch64` channel explicitly |
| `externally-managed-environment` (PEP 668) | Debian 12 hardening | `--break-system-packages` or venv |
| Servo jitter during teleop | noise on TTL bus | added capacitor + shorter cable |
| ACT loss plateau | too few epochs, bad LR | 50 epochs, swept 1e-4 / 5e-4 / 1e-3 |

#### 📊 Result

> ✅ **The arm completes vision-based grasps autonomously** — my first end-to-end embodied loop on physical hardware.
...
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=zizhengwang2026&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub Stats"/>
</p>
